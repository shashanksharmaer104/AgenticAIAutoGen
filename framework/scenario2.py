...
async def main():
    ...
    database_agent = factory.create_database_agent(system_message=("""
            ...
            Your task/role:
            1. Connect with the MySQL database 'rahulshettyacademy'
            2. Query the 'RegistrationDetails' table to get a single random record
            3. Query the 'Usernames' table to get additional data 
            4. Combine the data from both tables to create complete registration information
            5. Ensure the email is unique by adding a timestamp or random number if required
            6. Prepare all the registration data in structured format so that another agent can understand
            
            When ready, write or print "DATABASE_DATA_READY - RestAPIAgent should proceed next"
    """))
    rest_api_agent = factory.create_rest_api_agent(system_message=("""
          You are an API testing specialist with access to both REST API tools and filesystem.
            ...
            3. Before making a registration API call- construct it's body field with the DatabaseAgent's data inline with below format rules:
                1. Email should be unique. Add timestamp/random data
                2. Password should be a format of SecurePass123
                3. Mobile number format - 1234567890
    ...
    team = RoundRobinGroupChat(
        participants=[database_agent, rest_api_agent, excel_agent],
        termination_condition=TextMentionTermination("REGISTRATION PROCESS COMPLETE")
    )
    await Console(team.run_stream(task="Execute Sequential User Registration Process:\n\n"
